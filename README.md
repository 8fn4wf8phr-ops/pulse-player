...
**20. Reordering and playlists.** The library was a flat list in the order files were added. Every row in the library panel now has a drag handle, and the order you set is saved (an `order` field on each IndexedDB record, backfilled for libraries from before it existed). Dragging uses pointer events on the handle rather than the HTML5 drag-and-drop API, because that API doesn't fire from touch on iPhone; the handle also responds to the up/down arrow keys. Playlists live in a second IndexedDB store (the database went to version 2, upgrading in place), chosen from a dropdown above the list and filled from a per-track playlist button; inside a playlist the same handle reorders that playlist's own track order, and the row's × removes the track from the playlist rather than deleting it from the library. Next, previous, and auto-advance follow whichever list the current track was started from, so a playlist actually plays as a playlist. Playlists travel with export and sync too: they refer to tracks by content hash (local ids differ between devices) and carry a stable id, the most recently edited copy wins, and a deleted playlist leaves a tombstone so an older copy on another device can't resurrect it. The custom track order syncs as one newest-wins value, by track hash, so a brand-new device gets your order in one shot and tracks it doesn't know about stay at the end. "Newest" is decided by a logical clock rather than raw device time: every edit (a rename, a playlist change, a delete, a reorder) is stamped no earlier than anything the device has already received, so an edit made after seeing another one always wins even if that device's clock runs slow, and exact ties are broken by comparing content so every device picks the same winner. Only edits made at the same moment on two devices fall back to wall-clock time.
